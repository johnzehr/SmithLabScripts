...
#    objFile = open("MetalSheet1m", "wb")
 #   pick.dump(data, objFile)
  ...
  #  objFile.close()
    ...
    # pickle.dump(exportDict,exportFile)
    # exportFile.close()
...
def matched_filter(waveFilePath,measFilePath,backgrounded=False):
    ...
    eps = 1.0e-16
    originalPulseData = waveDataFileDict.get("wave gen data")
    # speed of light
    c = 3.e8
    ...
    num_samp = waveDataFileDict.get("numberSamples")
    clock = waveDataFileDict.get("clock")
    taup = num_samp/clock
    ...
    fstart = waveDataFileDict.get("fstart")
    fstop = waveDataFileDict.get("fstop")
    chirp_rate = (fstop - fstart) / T
    signal = waveDataFileDict.get("originalPulse")
    ...
    data_arr = measDataFileDict.get('recorded data')
    # print(data_arr)
    # plt.plot(data_arr)
#   subtract background signal if it exists in dictionary
    if(backgrounded and measDataFileDict.get("background") != []):
        data_arr = np.subtract(data_arr,waveDataFileDict.get("background"))
    # # correlate signals
    # print(np.shape(replica))
    # print(np.shape(data_arr))
    
    plt.plot(np.fft.fft(replica))
    plt.plot(np.fft.fft(data_arr))
    plt.legend(['Tx', 'Rx'])
    plt.show()
